...
extension CustomKeyboardViewController
{
    func showAccessView()
    {
        ...
        viewAccess.tag = VIEWINSTACCESSTAG
...
//        let filePath = NSBundle.mainBundle().pathForResource("Access", ofType:"png")
        // get the image from the path
        let image = UIImage(named: "Access.png")
        imgView.image = image
        ...
        btnClose.addTarget(self, action: "onCloseInstAccessView:", forControlEvents: .TouchUpInside)
    ...
    func onCloseInstAccessView(sender:UIButton!)
    {
        addedHeight = 0
        changeKeyboardHeight()
        
        view.viewWithTag(VIEWINSTACCESSTAG)!.removeFromSuperview()
        showKeyboard()
    }
}
