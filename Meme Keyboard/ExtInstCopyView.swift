...
extension CustomKeyboardViewController
{
    func showInstCopyView()
    {
        ...
        viewAccess.tag = VIEWINSTCOPYTAG
        ...
        let image = UIImage(named: "Copy.png")
        imgView.image = image
        ...
        btnClose.addTarget(self, action: "onCloseInstCopyView:", forControlEvents: .TouchUpInside)
    ...
    func onCloseInstCopyView(sender:UIButton!)
    {
        addedHeight = 0
        changeKeyboardHeight()
        
        view.viewWithTag(VIEWINSTCOPYTAG)!.removeFromSuperview()
        showCopyView()
    }
}
